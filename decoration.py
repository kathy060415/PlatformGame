...
class Sky:
    def __init__(self, horizon, style='level'):
        ...
        self.horizon = horizon  # point tiles should switch
    ...
    def draw(self, surface):
        for row in range(vertical_tile_number):
            y = row * tile_size
            if row < self.horizon:
                surface.blit(self.top, (0, y))  # fills surface
...
class Water:
    def __init__(self, top, level_width):   # top = how high water will be
        ...
        tile_x_amount = int((level_width + screen_width * 2) / water_tile_width)  # number of water tiles needed to fill space
        self.water_sprite = pygame.sprite.Group()

        for tile in range(tile_x_amount):
            x = tile * water_tile_width + water_start
            y = top
            sprite = AnimatedTile(192, x, y, 'Treasure Hunters/Treasure Hunters/Palm Tree Island/Sprites/decoration/water')
...
class Clouds:
    def __init__(self, horizon, level_width, cloud_number):
        cloud_surf_list = import_folder('Treasure Hunters/Treasure Hunters/Palm Tree Island/Sprites/decoration/clouds')
        min_x = -screen_width
